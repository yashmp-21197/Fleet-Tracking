...
pushLocation = (vehiclekey) => {
    ...
    BackgroundGeolocation.getLocations( //getValidLocations
        function (locations) {            
            var location = {
                time : time,
                lat : locations[locations.length-1].latitude,
                long : locations[locations.length-1].longitude,
//                id : locations[locations.length-1].id,
            };
//            Alert.alert(
//                locations.length.toString()+","+locations[locations.length-1].id.toString(),
//                JSON.stringify(locations[locations.length-1]));
            firebase.database()
